...
export class ConvertiInFiat implements OnInit {
  ...
  convert() {
    ...
    this.api.convertFiat(this.accountId, { to: this.targetCurrency }).subscribe({ next: (res:any) => { this.result = res; this.loading = false; }, error: (e:any) => { this.error = e?.message || 'Conversion error'; this.loading = false; } });
  }
}
