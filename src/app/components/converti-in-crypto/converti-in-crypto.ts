...
export class ConvertiInCrypto implements OnInit {
  ...
  convert() {
    ...
    this.api.convertCrypto(this.accountId, { to: this.target }).subscribe({ next: (res:any) => { this.result = res; this.loading = false; }, error: (e:any) => { this.error = e?.message || 'Conversion error'; this.loading = false; } });
  }
}
